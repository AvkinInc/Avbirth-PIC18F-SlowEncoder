...
# 53 "step/../mcc_generated_files/mcc.h" 2
...
# 71 "step/../mcc_generated_files/mcc.h"
void SYSTEM_Initialize(void);
# 84 "step/../mcc_generated_files/mcc.h"
...
uint16_t control_bits = 0xC10;
uint8_t control_bits_read_high = 0;
uint8_t control_bits_read_low = 0;

void initDRV8711(void){
# 28 "step/step.c"
    DRV8711_SPI_CMD(0x01, 0x1FF);

    DRV8711_SPI_CMD(0x02, 0x030);

    DRV8711_SPI_CMD(0x03, 0x080);

    DRV8711_SPI_CMD(0x04, 0x110);

    DRV8711_SPI_CMD(0x06, 0xA59);

    DRV8711_SPI_CMD(0x05, 0x040);

    DRV8711_SPI_CMD(0x00, 0xC10);


    DRV8711_SPI_CMD(0x07, 0);


    DRV8711_SPI_CMD(0x04, 0);
# 66 "step/step.c"
    uint8_t isgainBits = 0b11;
    uint16_t torqueBits = ((uint32_t)768 * 1250) / 6875;
# 77 "step/step.c"
    control_bits = (control_bits & 0b110011111111) | (isgainBits << 8);
    DRV8711_SPI_CMD(0x00, control_bits);
    uint16_t torque = (0x1FF & 0b111100000000) | torqueBits;
    DRV8711_SPI_CMD(0x01, torque);
...
void DRV8711_SPI_CMD(uint8_t addr, uint16_t cmd){
    ...
    uint8_t addr4 = (addr & 0b0111);
    uint16_t cmd12 = (cmd & 0xFFF);
...
void DRV8711_SPI_read(uint8_t addr){
    ...
    uint8_t addr4 = (addr | 0b1000);
    do { LATCbits.LATC2 = 1; } while(0);

    control_bits_read_high = SPI1_ExchangeByte((addr4<<4) & 0b11110000);
    control_bits_read_low = SPI1_ExchangeByte(0);
# 128 "step/step.c"
    do { LATCbits.LATC2 = 0; } while(0);
    writeString("w:");
    writeByte('\t');
...
void Step(direction dir, uint16_t num_steps, step_size size) {
    ...
    DRV8711_SPI_read(0x00);

    uint16_t stepsCounter;
    for (stepsCounter = 0; stepsCounter < num_steps; stepsCounter++) {
        if (0) {
...
void inchForwards(step_size size) {
    Step(FORWARDS, (200 * 2 * getMultiplier(size)), size);
}

void inchBackwards(step_size size) {
    Step(BACKWARDS, (200 * 2 * getMultiplier(size)), size);
}
